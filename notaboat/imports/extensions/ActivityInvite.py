...
class ActivityInvite(commands.Cog):
    ...
    async def start_activity(self, interaction: discord.Interaction, channel: discord.VoiceChannel, activity: discord.Object) -> None:
        await interaction.response.send_message(f"**Channel:** {channel.mention}\n**Activity ID:** {activity.id}", ephemeral=True)
        return
